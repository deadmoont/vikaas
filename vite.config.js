...
const CERT_FILE = resolve(__dirname, "certs/hakarrrank.com.pem");
const KEY_FILE = resolve(__dirname, "certs/hakarrrank.com-key.pem");
...
if (!hasCerts) {
  console.log('No local HTTPS cert found — run "npm run setup-https" for https://hakarrrank.com/ with no browser warning.');
}
...
export default defineConfig({
  ...
  server: {
    ...
    open: hasCerts ? "https://hakarrrank.com/" : "http://hakarrrank.com/",
    ...
    host: true,
    allowedHosts: ["hakarrrank.com"],
    https: hasCerts ? { cert: readFileSync(CERT_FILE), key: readFileSync(KEY_FILE) } : undefined,
  },
});
