// One-time local setup: maps the custom dev hostname (http://hackerrrank.com —
...
function alreadyMapped(contents) {
  return contents
    ...
    .some(
      (line) =>
        !line.trim().startsWith("#") && line.split(/\s+/).includes(HOST),
    );
}
...
try {
  ...
  if (alreadyMapped(contents)) {
    console.log(
      `✓ ${HOST} is already mapped to 127.0.0.1 in ${HOSTS_FILE} — nothing to do.`,
    );
    process.exit(0);
  }
  ...
  console.log(
    `  Run "npm run dev" — it'll open http://${HOST}/ automatically.`,
  );
} catch (err) {
